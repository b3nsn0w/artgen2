...
const artFractal = (gen, name) => {
  ...
  const getLevels = zoom => new Array(3)
    .fill()
    .map((v, i) => zoom + i)
    .reduce((acc, z) => {
  ...
  const render = ({ canvas, context, origin, scale, zoom, palette }) => {
    ...
    const zoomScale = scale * Math.pow(step, zoom)
    console.log(zoomScale, levels)

    for (const level in levels) {
      const currentScale = zoomScale / Math.pow(step, Number(level))
