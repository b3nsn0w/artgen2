...
const luna = (gen, name) => {
  ...
  const render = ({ context, origin, scale, palette }) => {
    ...
    drawCircle(context, { x: 0, y: 0 }, radius, shapeGen.from('luna-color', palette))
    rings.map(ring => {
      drawCircle(context, ring.origin, ring.size, shapeGen.from({ 'luna-ring-color': ring.number }, palette))
    })
