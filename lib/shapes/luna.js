...
const luna = (gen, name) => {
  ...
  const render = ({ context, origin, scale, palette }) => {
    ...
    drawCircle(context, { x: 0, y: 0 }, radius, picker(shapeGen, 'luna-color', context, palette))
    rings.map(ring => {
      drawCircle(context, ring.origin, ring.size, picker(shapeGen, { 'luna-ring-color': ring.number }, context, palette))
    })
